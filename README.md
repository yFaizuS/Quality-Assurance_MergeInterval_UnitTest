...
## UI Test using Selenium Web Driver


Selenium WebDriver is an open-source framework used for automating web browsers. It provides a programming interface that allows developers to interact with web browsers and automate tasks like navigating to web pages, filling out forms, and extracting data. With support for multiple programming languages, it is widely used for web testing and automation.

Using Selenium WebDriver, developers can write scripts or code that controls web browsers, simulating user actions and automating repetitive tasks. It follows a client-server architecture, where the client library communicates with the browser through a browser-specific driver.

The WebDriver API offers a range of methods to control the browser, locate elements on web pages, interact with them, and handle alerts and pop-ups. By setting up the appropriate browser driver, developers can establish a connection between their code and the browser they want to automate, enabling seamless interaction.

In summary, Selenium WebDriver is a powerful tool that allows developers to automate web browsers, perform web testing, and build automation workflows. Its flexibility, cross-browser compatibility, and extensive features make it a popular choice for web automation tasks.

code for the UI Selenium test in :
`MergerInterval_UI Test(Selenium).js`
